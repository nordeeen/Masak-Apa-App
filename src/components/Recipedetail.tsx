'use client';

import { useState } from 'react';
import { useAppStore } from '../store/useAppStore';
import type {
  RecipeDetail as RecipeDetailType,
  MenuSuggestion,
} from '../types';

interface Props {
  recipe: RecipeDetailType;
  menu: MenuSuggestion;
}

export default function RecipeDetail({ recipe, menu }: Props) {
  const { saveRecipe, unsaveRecipe, isRecipeSaved, reset } = useAppStore();
  const [saved, setSaved] = useState(() => isRecipeSaved(menu.id));

  function toggleSave() {
    if (saved) {
      unsaveRecipe(menu.id);
      setSaved(false);
    } else {
      saveRecipe(recipe, menu);
      setSaved(true);
    }
  }

  const youtubeUrl = `https://www.youtube.com/results?search_query=${encodeURIComponent(
    recipe.youtubeSearchQuery,
  )}`;

  return (
    <div>
      {/* Header card */}
      <div className="bg-card border border-border rounded-2xl p-6 mb-4 flex gap-5 items-start">
        <div className="text-6xl leading-none shrink-0">{recipe.emoji}</div>
        <div className="flex-1 min-w-0">
          <h2 className="font-display text-3xl text-text-primary leading-tight mb-2">
            {recipe.name}
          </h2>
          <p className="text-sm text-text-secondary leading-relaxed mb-4">
            {recipe.description}
          </p>
          <div className="flex gap-5">
            <Stat value={`⏱ ${recipe.estimatedTime}`} label="Waktu" />
            <Stat value={`${recipe.servings} porsi`} label="Porsi" />
            <Stat value={recipe.difficulty} label="Level" />
          </div>
        </div>
      </div>

      {/* Bahan + Langkah */}
      <div className="grid grid-cols-1 md:grid-cols-[1fr_1.6fr] gap-4 mb-4">
        {/* Bahan */}
        <div className="bg-card border border-border rounded-2xl p-5">
          <h3 className="text-xs font-semibold text-text-muted uppercase tracking-wider mb-4">
            🛒 Bahan-Bahan
          </h3>
          <ul className="space-y-0">
            {recipe.ingredients.map((ing, i) => (
              <li
                key={i}
                className="flex items-center gap-3 py-2.5 border-b border-border last:border-none text-sm"
              >
                <span className="w-1.5 h-1.5 rounded-full bg-accent shrink-0" />
                <span className="flex-1 text-text-primary">{ing.name}</span>
                <span className="text-text-muted text-xs">{ing.amount}</span>
                {ing.isUserHave && (
                  <span className="text-[10px] font-semibold text-green-400">
                    ✓
                  </span>
                )}
              </li>
            ))}
          </ul>
        </div>

        {/* Langkah */}
        <div className="bg-card border border-border rounded-2xl p-5">
          <h3 className="text-xs font-semibold text-text-muted uppercase tracking-wider mb-4">
            📝 Cara Memasak
          </h3>
          <ol className="space-y-4">
            {recipe.steps.map((step, i) => (
              <li
                key={i}
                className="flex gap-3 text-sm text-text-secondary leading-relaxed"
              >
                <span className="w-6 h-6 rounded-full bg-accent text-black text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">
                  {i + 1}
                </span>
                <span>{step}</span>
              </li>
            ))}
          </ol>
        </div>
      </div>

      {/* Tips */}
      {recipe.tips && (
        <div className="flex gap-3 items-start p-4 bg-accent/5 border border-accent/15 rounded-xl mb-4 text-sm text-text-secondary leading-relaxed">
          <span className="text-base shrink-0">💡</span>
          <span>{recipe.tips}</span>
        </div>
      )}

      {/* YouTube */}
      <a
        href={youtubeUrl}
        target="_blank"
        rel="noopener noreferrer"
        className="flex items-center gap-4 p-4 bg-card border border-border rounded-xl hover:border-red-500/40 hover:-translate-y-0.5 transition-all duration-200 mb-4 no-underline group"
      >
        <div className="w-20 h-14 bg-surface rounded-lg flex items-center justify-center text-2xl border border-border shrink-0">
          ▶️
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-text-primary mb-1 truncate">
            Tutorial: {recipe.name}
          </p>
          <p className="text-xs text-text-muted">
            Klik untuk tonton cara memasak di YouTube
          </p>
        </div>
        <div className="flex items-center gap-1.5 bg-red-500 text-white text-xs font-bold px-3 py-1.5 rounded-lg shrink-0">
          ▶ YouTube
        </div>
      </a>

      {/* Actions */}
      <div className="flex gap-3 items-center">
        <button
          onClick={toggleSave}
          className={`flex items-center gap-2 px-5 py-3 rounded-xl border text-sm font-medium transition-all duration-200 whitespace-nowrap ${
            saved
              ? 'border-red-400/40 text-red-400 bg-red-400/5'
              : 'border-border text-text-secondary bg-card hover:border-red-400/40 hover:text-red-400'
          }`}
        >
          {saved ? '♥ Tersimpan' : '♡ Simpan Resep'}
        </button>
        <button
          onClick={reset}
          className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-accent hover:bg-accent-light text-black font-semibold text-sm rounded-xl transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-accent/20"
        >
          🔄 Cari Menu Baru
        </button>
      </div>
    </div>
  );
}

function Stat({ value, label }: { value: string; label: string }) {
  return (
    <div>
      <div className="font-display text-lg text-accent">{value}</div>
      <div className="text-[10px] text-text-muted uppercase tracking-wider mt-0.5">
        {label}
      </div>
    </div>
  );
}
